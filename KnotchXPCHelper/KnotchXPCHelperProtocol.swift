...
@objc protocol KnotchXPCHelperProtocol {
...
/*
 To use the service from an application or other process, use NSXPCConnection to establish a connection to the service by doing something like this:
     ...
     connectionToService = NSXPCConnection(serviceName: "seshyweshyy.Knotch.KnotchXPCHelper")
     connectionToService.remoteObjectInterface = NSXPCInterface(with: (any KnotchXPCHelperProtocol).self)
     connectionToService.resume()

 Once you have a connection to the service, you can use it like this:

     if let proxy = connectionToService.remoteObjectProxy as? KnotchXPCHelperProtocol {
